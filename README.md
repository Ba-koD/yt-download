...
스토어 밖 확장이라 넣는 길이 둘이다. 둘 다 **확장 관리자**가 맡는다.

**자동 설치**(윈도우, 권장). 관리자에서 브라우저를 고르고 *이 브라우저에 자동 설치*를 누르면
브라우저 정책에 확장을 걸어둔다. 브라우저를 껐다 켜면 스스로 받아 깔고 그 뒤로도 스스로 갱신한다.
개발자 모드도, 폴더를 고르는 일도, 관리자 권한도 필요 없다. 대신 그동안 브라우저가
"조직에서 관리함"으로 표시되고, 확장을 브라우저 화면에서 지울 수 없다(관리자에서 해제한다).

**폴더로 얹기**(그 밖의 OS). 처음 한 번만 손으로 얹으면 그 뒤 갱신은 자동이다 —
관리자가 로그인 때 새 버전을 확인해 폴더를 갈아 끼우고, 확장이 스스로 다시 켜진다.
