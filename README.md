...
- 브라우저를 고르면(기본 브라우저가 미리 골라져 있습니다) 그 브라우저의 확장 페이지 주소와
  폴더 경로를 클립보드에 넣어줍니다
- **한 번 설치하면 그 뒤론 손댈 것이 없습니다.** 관리자가 로그인할 때 창 없이 조용히
  깃허브를 확인하고, 새 버전이 있으면 폴더를 갈아 끼웁니다. 확장은 그것을 스스로 알아채고
  다시 켜지며 열려 있던 유튜브 탭에도 새 판을 넣습니다(`chrome://extensions` 새로고침도
  F5 도 필요 없습니다). 바뀐 내용은 브라우저를 열어 보여줍니다.
  이 자동 확인은 처음 설치할 때 켜지고, 관리자 창에서 끌 수 있습니다(관리자 권한 불필요).

처음 한 번 폴더를 넣는 일만 사람이 합니다. 스토어 밖 확장의 자동 설치는 브라우저들이
일부러 막아둔 자리라 우회가 없습니다(기업 정책은 관리자 권한이 필요하고, 레지스트리 외부
설치는 웹 스토어 것만 허용하며, 원격 디버깅은 크롬 136부터 기본 프로필에 붙지 않습니다).
...
숏츠는 화면 생김새가 달라서 버튼이 오른쪽 세로 줄에 들어가고 패널은 화면 아래에 뜹니다.

지금은 mp4 화질(H.264 · AV1 4K)만 되고 비공개 영상은 아직 안 됩니다.
자세한 내용과 한계는 [`extension/README.md`](extension/README.md) 를 보세요.

## 자동 업데이트

앱과 확장 관리자는 스스로 최신 릴리스를 확인하고 갱신합니다.

- 앱: 오른쪽 위 **업데이트 확인** → **v0.3.0 받기** → **다시 켜기**
- 관리자: 창을 열면 확장과 관리자 자신을 함께 확인하고, 각각 따로 갱신합니다

지키는 것:

1. 다 받아서 검사한 **뒤에** 바꿔 끼웁니다. 실패하면 쓰던 실행 파일이 그대로 남습니다.
2. 관리자 권한을 요구하지 않습니다.
3. 받은 파일을 `SHA256SUMS.txt` 와 대조합니다.
4. 별도 창이나 도우미 프로세스가 뜨지 않습니다.
   윈도우에서 돌고 있는 파일은 지울 수 없지만 이름은 바꿀 수 있다는 성질을 씁니다
   ([`self-replace`](https://docs.rs/self-replace)).
5. 네 플랫폼에서 같은 방식으로 됩니다. macOS 는 `.app` 안의 실행 파일과 `Info.plist` 를
   함께 바꿉니다.

앱과 관리자는 릴리스 자산이 **따로**입니다(`yt-download-<플랫폼>`,
`yt-download-manager-<플랫폼>`). 관리자만 고쳐도 165MB 를 다시 받는 일이
없도록 나눠뒀습니다. 버전은 함께 갑니다 — `VERSION` 하나가 셋을 다 정합니다.

관리자는 어디서도 압축하지 않고 실행 파일 그대로 올립니다. 받아서 바로 실행하면 됩니다.
리눅스·macOS 에서는 한 번만 `chmod +x` 가 필요합니다(깃허브 릴리스 자산은 실행 권한을
잃습니다). 그 뒤의 갱신은 관리자가 스스로 권한을 주므로 신경 쓸 일이 없습니다.
...
scripts/
  ...
  make-logo.py            로고를 그려서 필요한 크기·형식으로 내보내기
